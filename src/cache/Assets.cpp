...
void Assets::Load( Progressive& prog ) {
    ...
    for (int i = 0; i<3; i++) {
        ...
        Assets::BTN_SPR[i]->setPosition( {40.0f, 80.f*(i+1) + 10} );
    ...
    Assets::scoreSprite1->setTextureball( *(Assets::SCORE_TEXTS[0]) );
    ...
    scoreSprite->setOrigin(
        sf::Vector2f(scoreSprite->getTexture().getSize()) / 2.f
    );
    ...
    back to original design somehow.
    Is it better to return copies of local sprites than to return that sprite itself?
    bundles/sub=struct
    ...
    scoreSprite1->setOrigin(
        sf::Vector2f(scoreSprite1->getTexture().getSize()) / 2.f
    );
    ...
    countDSprite->setOrigin(
        sf::Vector2f( countDSprite->getTexture().getSize()) / 2.f
    );
    ...
    SettSpr->setOrigin(
        sf::Vector2f(SettSpr->getTexture().getSize()) / 2.f
    );
    SettSpr->setPosition( Utils::W_CTR );

    ShadSpr->setTexture( *ShadTxt );
    ShadSpr->setOrigin(
        sf::Vector2f(ShadSpr->getTexture().getSize()) / 2.f
    );
...
sf::Rect<int> Assets::getButtonBound( const int id ) const {
    ...
    return sf::IntRect(
        ...
        { static_cast<int>( btnPtr->getGlobalBounds().size.x ),
        static_cast<int>( btnPtr->getGlobalBounds().size.y ) }
    );
}
